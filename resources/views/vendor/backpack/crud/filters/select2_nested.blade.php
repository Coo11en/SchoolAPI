{{-- Select2_nested Backpack CRUD filter --}}
{{--Передавать в фильтр нужно отсортированные по lft сущности в виде массива вместе с уровнем, так как по нему прорисовывается список --}}
{{--Использоваться как select2 фильтр, изменяя передаваемые options--}}
{{--Пример Category::active()->orderBy('lft')->select('id', 'name', 'depth')->get()->toArray(); --}}
				...
				@if (is_array($filter->values) && count($filter->values))
					@foreach($filter->values as $value)
						<option value="{{ $value['id'] }}"
							@if($filter->isActive() && $filter->currentValue == $value['id'])
								selected
							@endif
							>
							{{ str_repeat("-", (int)$value['depth'] > 1 ? (int)$value['depth'] - 1 : 0).' '.$value['name'] }}
						</option>
					@endforeach
				@endif
			</select>
